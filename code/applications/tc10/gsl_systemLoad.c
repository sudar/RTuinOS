...
/**
 * A diagnostic function, which can be used to estimate the system load. The function
 * executes some test code of known CPU execution time and relates this known time to the
 * measured, actually elapsed world time. The function needs to be called from the idle
 * task only. Now the mentioned ratio is a measure for the system load: The less the system
 * idles to longer it'll take to exceute the test code.\n
 ...
 *   @see uint8_t otherFunction(uint16_t)
 */

uint8_t gsl_getSystemLoad()
{
#define TI_WINDOW_LEN   1000ul  /* Should contain a full cycle of tasks activations */
...
/* The implementation of delayMicroseconds is buggy. It permits a uint16_t input but will
   shift the parameter twice to the left. An overrun occurs for large numbers. A limitation
   of the parameter is however not documented. */
#if TI_STEP*1000u > 0x4000 
# error TI_STEP too large, we run into a bug in delayMicroseconds
#endif
        /* One step is exactely 16 ms of code execution time - regardless of how long this
...
//Serial.print("step: ");
//Serial.println(step);
//Serial.print("tiStart: ");
//Serial.println(tiStart);
//Serial.print("tiEndMin: ");
//Serial.println(tiEndMin);
//Serial.print("tiEnd: ");
//Serial.println(tiEnd);

    
    /* If the system is idle the number of steps reaches its maximum of about 64. */
    ...
    /* If the elapsed time is too large, we can limit the result by rounding to 100%. The
       chosen boundary is to round all above 99.5% to 100%. Since we use 0.5% as resolution
       of the result, the rounded range can anyway not be distinguished from 100%. */
    if(tiEnd >= 200*tiStart)
        return 200; 
    else if(tiEnd <= tiStart)
    {
        /* Theoretically, the consumed CPU time can't be greater than the measured, elapsed
           time. However, we have some limitations of the accuracy, e.g. the accuracy of
           the delay function, so that this rule could be hurt. We need a limiting
           statement to safely avoid an overrun. */
        return 0;
    }    
    else
    {
        /* Normal situation. The load is (Consumed CPU time - Elapsed time)/Elapsed time.
           This is taken by 200 to get the desired resolution of 0.5% which lets the result
           safely fit into a uint8_t. */
