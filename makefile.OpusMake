...
# IDE and compile one of the code examples. Copy the contents of the IDE's output window
# and paste into a text editor. You will find appropriate command lines for all the tools.
...
h help targets usage .ALWAYS:
    ...
    %echo and \<portName\> is an a USB port identifying string to be used for the upload.
    ...
    %echo   upload: Build first, then flash the device
...
# Upload compiled software to the controller.
upload .ALWAYS: $(project).hex $(ARDUINO_HOME)\hardware\tools\avr\etc\avrdude.conf
