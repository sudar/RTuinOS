...
AVR \uC). At runtime this is the program memory location where to continue
...
task can barely operate with timeouts, e.g. if it needs to wait for events
posted by other tasks. The only timeout condition it can use is to state
the next regular due time. Nonetheless, reasonable error handling code is
still possible as the task code can distinguish between becoming active
because of the timeout or because it's its normal due time.
...
to the absolute timer demands to be resumed in the past.
...
and will not see a task overrun as 100-45 and 100-67 both is positive in
the chosen 8 Bit arithmetics. Given a bad, too long task execution time of
e.g. 102~ms \rtos{} would recognize a task overrun as 100-102 is negative.
...
execution time is the negative number -121 in the chosen 8 Bit
arithmetics. For \rtos{} the demanded resume time is seemingly in the past
and decides on a task overrun.
...
If we follow the recommendation made before this means that the ratio of
...
tics later, i.e. at 210. The demanded next resume time is 123+100 = 223, which
...
Here's a short summary of the timer data type discussion:
\begin{itemize}
