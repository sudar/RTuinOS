...
After any \ident{rtos\_leaveCriticalSection}, all interrupts are surely
...
configuration file \refRTOSConfigTemplateH{} into this folder and rename
it to \ident{rtos.config.h}.
...
Create a static array of \ident{uint8\_t} of appropriate size for each
...
Create the empty function \ident{loop}.
...
e.g. \ident{println} returns only after all bytes have been processed.
...
When \rtos{}' \ident{setup} is executed, no multi-tasking is active yet.
...
of code existing in \ident{rtos\_setEvent} and \ident{rtos\_waitForEvent}.
...
Furthermore, using \ident{rtos\_enter/leaveCriticalSection}, an \rtos{}
