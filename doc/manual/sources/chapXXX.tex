...
applications. All of these can be build and downloaded using the makefile
...
Another prerequiste of successfully running the makefile is that your
...
An optional, final rule permitts to download the binary code onto the Arduino
board. If you have the Arduino IDE (arduino.exe) open, all you still have
to do to make your application run is a ALT-TAB to switch to the IDE and a
Ctrl-Shft-m to open the console window.
...
depent on the compilation configuration.
...
source code directory \ident{code\textbackslash\-RTOS\textbackslash} and a
second, variable directory. This directory is located at
\ident{code\textbackslash\-applications\textbackslash} but its name is
provided by macro \ident{APP}. This enables you to select an application
for compilation simply by stating \verb+APP=myRTuinOSApplication+ on the
...
during development, when configuration \ident{DEBUG} is used.


\subsection{Switching Applications}

Any \rtos{} application will use its own configuration; it'll have its own
customized copy of \ident{rtos.config.h}. The compilation of \rtos{}
strongly depends on this file. If you switch the application in
subsequent calls of make you will need to force make to recompile \rtos{}.

Basically, switching work from one application to another, just means to
change the value of macro \ident{APP} on the command line of make, but
please, always start your work on another application with a \ident{clean}
or \ident{rebuild} in order to avoid running ito severe, hard to find trouble!

\subsection{Weaknesses of the Makefile}

\subsubsection{Dependencies}

In the current state, the compiler generated *.d files are not integrated
into the build process. The *.d files are makefile rules, which describe
the dependencies of object files on source files. Doing this integration
is one of the most important TODOs for the makefile as it currently
disregards all non-trivial dependencies: Only the immediate dependency of
\ident{myModule.o} on \ident{myModule.c} is considered, or on
\ident{myModule.cpp} respectively.

Consequently, in all cases of non-trivial changes of header files you
should always call the rules to rebuild the application. This is also
mandatory if you switch from compiling one \rtos{} application (or test
case) to compiling another one.

\subsubsection{Building the required Output Directories}

If you compile an \rtos{} application the very first time, the dircetories
...
\section{Configurating the System Timer}
...
TODO: Events in queues? Useful, application can implemenet only as polled
queues, significantly widens the implementation, degrades the teaching
character the implementation curently has - therefore not done, but should
be possible by anybody who understands the current implementation
