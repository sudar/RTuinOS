...
function \ident{rtos\_setEvent} and the latter task will resume.
...
\ident{rtos\_setEvent} and the suspend commands. From the bird's eye view
on the system task code switching appears as follows: The suspend function
is invoked but it returns out of a call of \ident{rtos\_setEvent} of
another task or out of the suspend command of a task which became due
meanwhile. If a task calls \ident{rtos\_setEvent} it doesn't need to
...
a C++ class embedding all calls of \ident{rtos\_waitForEvent} and
\ident{rtos\_setEvent} into member functions like \ident{acquireMutex} and
\ident{releaseMutex}. Each task would have its own, local object of this
class. However, this concept is simple only if waiting for combinations
of events is excluded -- which won't be a painful restriction for most
applications.}
...
with the number of instances from a managed pool of resources the task
was granted access to. The number of instances a task has accumulated
across a series of \ident{rtos\_waitForEvent} and \ident{rtos\_setEvent}
...
\ident{rtos\_setEvent} into member functions like \ident{acquireSemaphore}
...
the hidden call of \ident{rtos\_setEvent} --, which actually serves as
...
\ident{rtos\_setEvent}). Both machine operations start execution with
...
\ident{rtos\_setEvent} is not shown; it is implemented like an ISR, it
...
is added to member \ident{postedEventVec} of the suspended task. This
...
or from an ISR or \ident{rtos\_setEvent} (which must return with restored
...
will release it using \ident{rtos\_setEvent}. Or immediately, if no other
...
An event can be a semaphore, a synchronization object for managing access
to a pool of resources shared between different tasks. The kind of an
event is specified at compile time, it's part of the configuration of
\rtos.
...
\ident{rtos\_setEvent}. Or immediately, if there's currently at least one
...
\ident{rtos\_setEvent}. Since wait conditions can be complex, the return
...
The function \ident{rtos\_getTaskOverrunCounter} may be called by any task,
...
variable \ident{ARDUINO\_HOME} needs to point to your Arduino installation (see
...
tool as
arduino-1.0.5\textbackslash\-hard\-ware\textbackslash\-tools\textbackslash\-avr\textbackslash\-utils\textbackslash\-bin\textbackslash\-make.exe.
...
c:\textbackslash{}ProgramFiles\textbackslash\-arduino-1.0.5. You will
...
\ident{rtos\_setEvent} would probably be the best fitting starting point
...
micro controller. By modifying \ident{prepareTaskStack} it could become any other
...
\ident{gsl\_getSystemLoad} from the idle task. Please refer to
\refGSLSystemLoadH{} and \refGSLSystemLoadC{} for details.
