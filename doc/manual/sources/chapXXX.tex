...
An optional, final rule permitts to download the binary code onto the
...
please, in order to avoid running ito severe, hard to find trouble, always
...
impact on the acutal tree of nested include statements. Particularly in
...
customize the type in your application's copy of \ident{rtos.config.h}. In
...
slowest and fastest task. This maximum ratio is half the maxmum number,
...
handeld with a \ident{uint8\_t} (ratio 1:100). If you want to have an
...
\ident{RTOS\_DEFINE\_TYPE\_OF\_SYSTEM\_TIME}y, please refer to
\refRTOSConfigH.


\subsection{Configurating the System Timer Interrupt}

TODO weak function, to be overloaded, macro for interrupt vector, register
programming of peripherals, Arduino standard configuration not changed,
prescaler: ISR=n*systemTic
